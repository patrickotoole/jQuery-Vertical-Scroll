	...
	$.fn.scrollbar = function(method,user_options) {
			...
			create : function () {
				return this.each(function(index) {
					...
					{		
						...
						var new_c = jQuery("<div></div>").attr("class",cl).attr("id",id).css({'position': 'relative','overflow-y': 'hidden','overflow-x':'visible','width': w,'height': h});	
		...
		*/
		function WrapContent(current_content) {
			var s_content = jQuery(current_content).clone(true);
		...
		*/
		function RemoveScrollbar(c) {
			...
			if (original_content.length > 0) {
				c.html(original_content.html()).removeAttr("style");
				console.log(c.data());
		...
		*/
		function SliderToScrollbar(id,h) {
			...
			var scroll_outer = $("#" + id + " .scrollbar_outer"), 
				scroll_bar = scroll_outer.children(".scrollbar");
		...
		function GenerateScrollControls(id) {
			...
			options.scrollbar.incrementSize = (options.scrollbar.incrementSize) ? options.scrollbar.incrementSize : parseInt(scrollbar_v.height()/5);
	...
	$.fn.scrollbar.wheel = function (id,scrollStep,scrollDifference) {
		if (jQuery.isFunction(jQuery.fn.mousewheel)) {
			$("#"+id).mousewheel(function(event, amount){
				var slider_value = $('#' + id + ' > .scrollbar_outer > .scrollbar').slider("value");
